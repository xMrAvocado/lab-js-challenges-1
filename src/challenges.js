...
function howManyTimes(arrWords, wordSearch) {
  let sum = 0;

  if (arrWords.length === 0) {
    return 0;
  } else if (arrWords.includes(wordSearch)) {
...
function multiplyBy(arrNum2, num2) {
  let arrEmp = [];
  ...
  if (arrNum2.length === 0) {
    ...
    for(let i = 0; i < arrNum2.length; i++){
      let numMultip = arrNum2[i] * num2;
      arrEmp.push(numMultip);
    }
    return arrEmp;
  }
}
...
function filterOut(arrOr, arrFilt) {
  let arrFish = [];

  if (arrOr.length === 0) {
    return null;
  }else if(arrFilt.length === 0){
    return arrOr;
  }else{
    for (let i = 0; i < arrOr.length; i++) {
      for (let j = 0; j < arrFilt.length; j++) {
        if (arrFilt[j] === arrOr[i]) {
          arrOr.splice(i, 1);
        }
        arrFish = arrOr;
      }
...
function uniquifyArray(arrWords2) {
  ...
  for (let i = 0; i < arrWords2.length; i++) {
    if(arrDupl.includes(arrWords2[i])){
      continue;
    }else{
      arrDupl.push(arrWords2[i]);
    }
  }
  return arrDupl;
}
